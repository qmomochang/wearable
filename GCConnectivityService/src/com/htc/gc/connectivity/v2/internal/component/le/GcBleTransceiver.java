...
import com.htc.gc.connectivity.v2.internal.component.le.queue.GattQueueManager.GattRequest;
import com.htc.gc.connectivity.v2.internal.component.le.queue.GattQueueManager;
import com.htc.gc.connectivity.v2.internal.component.le.queue.GattQueueManager.IGattRequest;
...
public class GcBleTransceiver implements IGattRequest {
    ...
    private static final int PAYLOAD_SIZE = 18;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        	...
        	if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                // Test code
                printCharacteristic("WRITE", characteristic);
                ...
                for (GcBleTransceiverListener listener : listeners) {
                	...
                	listener.onCharacteristicWrite(gatt.getDevice(), characteristic);
                }
                ...
                for (GcBleTransceiverListener listener : listeners) {
                	...
                	listener.onError(gatt.getDevice(), characteristic, Common.GcBleTransceiverErrorCode.ERROR_CHARACTERISTIC_WRITE);
                }
        	}
        }
    ...
    private synchronized boolean setGattClient(BluetoothDevice device, boolean auto, BluetoothGattCallback callback, Context context) {
        ...
        BluetoothGatt gatt = device.connectGatt(context, auto, callback);
    ...
    public int writeGcCommand(BluetoothDevice device, String command, byte[] writeData, long delay) {
    	...
    	UUID uuidService = UUID.fromString(GcBleGattAttributes.GC_SERVICE);
    	UUID uuidChar = UUID.fromString(command);

        Log.d(TAG, "[MGCC] writeGcCommand uuidChar = " + uuidChar + ", length = " + writeData.length);
    	
        if (GcBleGattAttributes.isLongFormat(command)) {
        	
        	ArrayList<byte[]> writeDataList = getPayload(writeData);
    ...
    private ArrayList<byte[]> getPayload(byte[] writeData) {
    	
    	ArrayList<byte[]> retWriteDataPayload = new ArrayList<byte[]>();

    	int payloadSize = PAYLOAD_SIZE;
    	int payloadRes = writeData.length % payloadSize;
    	int payloadNum = (writeData.length / payloadSize) + ((payloadRes > 0) ? 1 : 0);
    	int offset = 0;
    	
    	///Log.d(TAG, "[MGCC] getPayload payloadSize = " + payloadSize);
    	///Log.d(TAG, "[MGCC] getPayload payloadRes AA = " + payloadRes);
    	///Log.d(TAG, "[MGCC] getPayload payloadNum = " + payloadNum);
    	...
    	///Log.d(TAG, "[MGCC] getPayload payloadRes BB = " + payloadRes);
    	...
    	for (int cnt = 0; cnt < payloadNum; cnt++) {
    		...
    		byte[] payload = new byte[res + 2];
    		
    		payload[0] = (byte) ((last << 7) | res);
    		...
    		for (int idx = 0; idx < res; idx++) {
    			...
    			payload[idx + 2] = writeData[cnt * payloadSize + idx];
    ...
    public int setGcNotification(BluetoothDevice device, String command, boolean enable, long delay) {
    	...
    	UUID uuidService = UUID.fromString(GcBleGattAttributes.GC_SERVICE);
    	UUID uuidChar = UUID.fromString(command);
        UUID uuidDescriptor = UUID.fromString(GcBleGattAttributes.GC_DESCRIPTOR);
