...
public class GcBleSetNotificationCallable implements Callable<BluetoothGattCharacteristic> {
	...
	private GcBleTransceiverListener mGcBleTransceiverListener = new GcBleTransceiverListener() {
		...
		public void onDescriptorWrite(BluetoothDevice device, BluetoothGattDescriptor descriptor) {
			...
			if (device.equals(mBluetoothDevice) && descriptor.getCharacteristic().getUuid().toString().equals(mUuidString)) {
	...
	public GcBleSetNotificationCallable(GcBleTransceiver transceiver, BluetoothDevice device, String uuidString, boolean enable) {
	...
	public BluetoothGattCharacteristic call() throws Exception {
		...
		do {
			...
			int retValue = mGcBleTransceiver.setGcNotification(mBluetoothDevice, mUuidString, mEnable, 0);
			if (retValue < 0) {
