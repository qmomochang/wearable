...
public class GcBleReceiveNotificationCallable implements Callable<BluetoothGattCharacteristic> {
	...
	public GcBleReceiveNotificationCallable(GcBleTransceiver transceiver, BluetoothDevice device, String uuidString) {

		this(transceiver, device, uuidString, DEFAULT_CALLABLE_TIMEOUT);
	}

	
	
	public GcBleReceiveNotificationCallable(GcBleTransceiver transceiver, BluetoothDevice device, String uuidString, int timeout) {

		mGcBleTransceiver = transceiver;
		mBluetoothDevice = device;
		mUuidString = uuidString;
		m_isLongFormat = GcBleGattAttributes.isLongFormat(mUuidString);

		if (m_isLongFormat) {
			
			mCollector = new LongCommandCollector(mBluetoothDevice, mUuidString);
		}
