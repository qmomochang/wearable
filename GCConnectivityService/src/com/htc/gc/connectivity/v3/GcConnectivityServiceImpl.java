...
import com.htc.gc.connectivity.v3.internal.tasks.GcLongTermEventTask;
...
public class GcConnectivityServiceImpl {
	...
	public GcConnectivityServiceImpl(Context context, Messenger messenger) {
		...
		try {
			...
			mGcBleTransceiver = new GcBleTransceiver(mContext, mBluetoothManager);
			mGcWifiTransceiver = new GcWifiTransceiver(mContext);
	...
	protected void registerLTEvent(BluetoothDevice device, GcBleGattAttributes.GcV2CommandEnum commandID) {
		
		mGcConnectivityLongTermEventTask.registerUuid(device, commandID);
	}

	
	
	protected void unregisterLTEvent(BluetoothDevice device, GcBleGattAttributes.GcV2CommandEnum commandID) {
		
		mGcConnectivityLongTermEventTask.unregisterUuid(device, commandID);
	}
