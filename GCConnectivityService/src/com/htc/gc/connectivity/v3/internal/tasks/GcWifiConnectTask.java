...
public class GcWifiConnectTask extends GcConnectivityTask {
	...
	private void connectWifi() throws Exception {
		...
		Future<BluetoothGattCharacteristic> futureB, futureC, futureD, futureE, futureF, futureG;
		...
		if ((futureB.get() == null) || (futureC.get() == null)) {

			sendMessage(false, null, Common.ERROR_GATT_WRITE);
			return;
		}
		
		futureD = mExecutor.submit(new GcBleReceiveNotificationCallable(mGcBleTransceiver, mBluetoothDevice, GcBleGattAttributes.GcV2CommandEnum.WIFI_CONFIG_STATUS_EVENT, 40000));

		byte rndValue = (byte) (Math.abs(mRnd.nextInt()) % 16);
		byte band = (byte) (((rndValue & 0x0f) << 4) | 0x01);
		byte[] cfgArray = {(byte) band, (byte) 4, (byte) 1};
		futureF = mExecutor.submit(new GcBleReliableWriteCallable(mGcBleTransceiver, mExecutor, mBluetoothDevice, GcBleGattAttributes.GcV2CommandEnum.WIFI_CONFIG_REQUEST, cfgArray));
		if (futureF.get() == null) {
