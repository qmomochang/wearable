...
public class GcCameraModeNotifyTask extends GcConnectivityTask {

	private final static String TAG = "GcCameraModeNotifyTask";
	...
	public GcCameraModeNotifyTask(GcBleTransceiver gcBleTransceiver, GcWifiTransceiver gcWifiTransceiver, Messenger messenger, ExecutorService executor, BluetoothDevice device, int action) {
	...
	private void sendMessage(boolean result) {
		...
		try {
			...
			if (mAction == ACTION_SET_LTEVENT) {
				...
				outMsg.what = IGcConnectivityService.CB_SET_CAMERA_MODE_LTEVENT_RESULT;

			} else if (mAction == ACTION_CLR_LTEVENT) {
				
				outMsg.what = IGcConnectivityService.CB_CLR_CAMERA_MODE_LTEVENT_RESULT;
			}
