...
public class GcGetBroadcastSettingTask extends GcGeneralPurposeCommandTask {

	private final static String TAG = "GcGetBroadcastSettingTask";
	
	private BluetoothDevice mBluetoothDevice;
	
	
	public GcGetBroadcastSettingTask(GcBleTransceiver gcBleTransceiver, GcWifiTransceiver gcWifiTransceiver, Messenger messenger, ExecutorService executor, BluetoothDevice device) {
	...
	public void execute() throws Exception {
		...
		if (!read(mBluetoothDevice, APP_ID_BROADCAST, "pf", "onoff")) {
			Log.d(TAG, "[MGCC] read onoff fail");
			sendFailMessage(mReadError);
			return;
		}
		
		BroadcastSetting broadcastSetting = mReadValue.equals("1") ? BroadcastSetting.BROADCAST_SETTING_ON : BroadcastSetting.BROADCAST_SETTING_OFF;
		sendSuccessMessage(broadcastSetting);
	...
	private void sendFailMessage(int errorCode) {
		...
		try {
			...
			outMsg.what = IGcConnectivityService.CB_GET_BROADCAST_SETTING_RESULT;
	...
	private void sendSuccessMessage(BroadcastSetting broadcastSetting) {
		...
		try {
			...
			outData.putSerializable(IGcConnectivityService.PARAM_BROADCAST_SETTING, broadcastSetting);
