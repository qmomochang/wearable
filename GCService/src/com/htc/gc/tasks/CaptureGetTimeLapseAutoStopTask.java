...
import com.htc.gc.interfaces.IStillCapturer.TimeLapseAutoStopCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class CaptureGetTimeLapseAutoStopTask extends GCTask {
	private static final int COMMAND_ID = Protocol.CAPTURE_GET_TIMELAPSE_STOPSETTING;
	
	private final IStillCapturer mThat;
	private final TimeLapseAutoStopCallback mCallback;
	
	public CaptureGetTimeLapseAutoStopTask(IStillCapturer that, TimeLapseAutoStopCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			byte autoStop = bodyBuffer.get();
			mCallback.result(mThat, TimeLapseAutoStop.getKey(autoStop));
		}
