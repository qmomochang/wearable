...
import com.htc.gc.interfaces.IDeviceController.GetFakeShotSettingCallback;
import com.htc.gc.interfaces.IGCService.ICommandCancel;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetFakeShotSettingTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_FAKESHOOT;
	
	private final IDeviceController mThat;
	private final GetFakeShotSettingCallback mCallback;
	
	public GetFakeShotSettingTask(IDeviceController that, GetFakeShotSettingCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, FakeShotSetting.getKey(bodyBuffer.get()));
		}
