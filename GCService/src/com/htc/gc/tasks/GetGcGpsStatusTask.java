...
import com.htc.gc.interfaces.IDeviceController.GcGpsStatusCallback;
import com.htc.gc.interfaces.IGCService;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetGcGpsStatusTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_GPS_STATUS;

	private final IDeviceController mThat;
	private final GcGpsStatusCallback mCallback;
	
	public GetGcGpsStatusTask(IDeviceController that, GcGpsStatusCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, GcGpsStatus.getKey(bodyBuffer.get()));
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
