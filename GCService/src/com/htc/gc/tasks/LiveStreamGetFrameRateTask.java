...
import com.htc.gc.interfaces.ILiveViewer.LiveStreamFrameRateCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class LiveStreamGetFrameRateTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_LIVESTREAM_FRAMERATE;
	
	private final ILiveViewer mThat;
	private final LiveStreamFrameRateCallback mCallback;
	
	public LiveStreamGetFrameRateTask(ILiveViewer that, LiveStreamFrameRateCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, LiveStreamFrameRate.getKey(bodyBuffer.getShort()));				
			} catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
			}
