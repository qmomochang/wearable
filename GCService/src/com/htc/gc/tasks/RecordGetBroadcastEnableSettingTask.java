...
public class RecordGetBroadcastEnableSettingTask extends GCTask {
	private static final int COMMAND_ID = Protocol.VIDEO_GET_ENABLE_BROADCASTING;
	
	private final IVideoRecorder mThat;
	private final BroadcastEnableSettingCallback mCallback;
	
	public RecordGetBroadcastEnableSettingTask(IVideoRecorder that, BroadcastEnableSettingCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, IVideoRecorder.BroadcastEnableSetting.getKey(setting));
		}
