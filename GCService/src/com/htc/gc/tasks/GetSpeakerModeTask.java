...
import com.htc.gc.interfaces.IDeviceController.SpeakerMode;
import com.htc.gc.interfaces.IDeviceController.SpeakerModeCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetSpeakerModeTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_SPEAKER;

	private final IDeviceController mThat;
	private final SpeakerModeCallback mCallback;
	
	public GetSpeakerModeTask(IDeviceController that, SpeakerModeCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, SpeakerMode.getKey(bodyBuffer.get()));
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
