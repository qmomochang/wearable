...
import com.htc.gc.interfaces.ILiveViewer.GetLiveViewModeCallback;
import com.htc.gc.interfaces.ILiveViewer.LiveViewMode;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class LiveViewGetModeTask extends GCTask{
	private static final int COMMAND_ID = Protocol.SYS_GET_LIVEVIEW_MODE;
	
	private final ILiveViewer mThat;
	private final GetLiveViewModeCallback mCallback;
	
	public LiveViewGetModeTask(ILiveViewer that, GetLiveViewModeCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, LiveViewMode.getKey(bodyBuffer.get()));				
			} catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
			}
