...
import com.htc.gc.interfaces.IDeviceController.GetDebugLogSettingCallback;
import com.htc.gc.interfaces.IGCService;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetDebugLogEnableSettingTask extends GCTask {
	private static final int COMMAND_ID = Protocol.ENGINEER_GET_DEBUGLOG_ENABLE;
	
	private final IDeviceController mThat;
	private final GetDebugLogSettingCallback mCallback;
	
	public GetDebugLogEnableSettingTask(IDeviceController that, GetDebugLogSettingCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, DebugLogSetting.getKey(bodyBuffer.get()));				
			} catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
			}
