...
import com.htc.gc.interfaces.IStillCapturer.WbStatusCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class CaptureGetWbStatusTask extends GCTask {
	private static final int COMMAND_ID = Protocol.CAPTURE_GET_WB_STATUS;
	
	private final IStillCapturer mThat;
	private final WbStatusCallback mCallback;
	
	public CaptureGetWbStatusTask(IStillCapturer that, WbStatusCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, WbStatus.getKey(bodyBuffer.get()));
		}
