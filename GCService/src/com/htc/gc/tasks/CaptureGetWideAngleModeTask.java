...
import com.htc.gc.interfaces.IStillCapturer.WideAngleModeCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class CaptureGetWideAngleModeTask extends GCTask {
	private static final int COMMAND_ID = Protocol.CAPTURE_GET_WIDE_ANGLE_MODE;
	
	private final IStillCapturer mThat;
	private final WideAngleModeCallback mCallback;
	
	public CaptureGetWideAngleModeTask(IStillCapturer that, WideAngleModeCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, WideAngleMode.getKey(bodyBuffer.get()));
		}
