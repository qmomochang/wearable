...
import com.htc.gc.interfaces.IStillCapturer.TimeLapseFrameRateCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class TimeLapseGetFrameRateTask extends GCTask {
	private static final int COMMAND_ID = Protocol.CAPTURE_GET_TIMELAPSE_FRAMERATE;
	
	private final IStillCapturer mThat;
	private final TimeLapseFrameRateCallback mCallback;
	
	public TimeLapseGetFrameRateTask(IStillCapturer that, TimeLapseFrameRateCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, bodyBuffer.get());
		}
