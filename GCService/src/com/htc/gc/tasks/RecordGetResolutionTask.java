...
import com.htc.gc.interfaces.IVideoRecorder.ResolutionCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class RecordGetResolutionTask extends GCTask {
	private static final int COMMAND_ID = Protocol.VIDEO_GET_VIDEO_RESOLUTION;
	
	private final IVideoRecorder mThat;
	private final ResolutionCallback mCallback;
	
	public RecordGetResolutionTask(IVideoRecorder that, ResolutionCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, IVideoRecorder.VideoResolution.getKey(resolution));
		}
