...
import com.htc.gc.interfaces.IStillCapturer.FaceCountTimesCallback;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class CaptureGetFaceCountTimesSettingTask extends GCTask {
	private static final int COMMAND_ID = Protocol.CAPTURE_GET_FACEDETECT_TIMES;
	
	private final IStillCapturer mThat;
	private final FaceCountTimesCallback mCallback;
	
	public CaptureGetFaceCountTimesSettingTask(IStillCapturer that, FaceCountTimesCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, FaceCountTimes.getKey(bodyBuffer.get()));
		}
