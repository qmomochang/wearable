...
import com.htc.gc.interfaces.IDeviceController.GetPowerSaveModeCallback;
...
public class GetPowerSaveModeTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_POWERSAVMODE;

	private final IDeviceController mThat;
	private final GetPowerSaveModeCallback mCallback;
	
	public GetPowerSaveModeTask(IDeviceController that, GetPowerSaveModeCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, PowerSaveMode.getKey(bodyBuffer.get()));
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
