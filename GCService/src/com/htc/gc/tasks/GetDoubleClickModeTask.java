...
import com.htc.gc.interfaces.IDeviceController.DoubleClickModeCallback;
import com.htc.gc.interfaces.IGCService;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetDoubleClickModeTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_DOUBLE_CLICK_MODE;
	
	private final IDeviceController mThat;
	private final DoubleClickModeCallback mCallback;
	
	public GetDoubleClickModeTask(IDeviceController that, DoubleClickModeCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, DoubleClickMode.getKey(bodyBuffer.get()));				
			} catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
			}
