...
public class AutoBackupGetPreferenceTask extends GCTask {
	private static final int COMMAND_ID = Protocol.AUTOBACKUP_GET_PREFERENCE;
	
	private final IAutoBackuper mThat;
	private final AutoBackupPreferenceCallback mCallback;
	
	public AutoBackupGetPreferenceTask(IAutoBackuper that, AutoBackupPreferenceCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			mCallback.result(mThat, OptionCheck.getKey(bodyBuffer.get()), OptionCheck.getKey(bodyBuffer.get()), OptionCheck.getKey(bodyBuffer.get()));
		}
