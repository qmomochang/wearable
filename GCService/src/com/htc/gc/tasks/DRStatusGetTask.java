...
import com.htc.gc.interfaces.IDeviceController.DRStatus;
import com.htc.gc.interfaces.IDeviceController.StatusCallback;
import com.htc.gc.interfaces.IGCService.ICommandCancel;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class DRStatusGetTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_DR_STATUS;

	private final IDeviceController mThat;
	private final StatusCallback mCallback;

	public DRStatusGetTask(IDeviceController that, StatusCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, DRStatus.getKey(bodyBuffer.get()), bodyBuffer.getInt());
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
