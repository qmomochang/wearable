...
import com.htc.gc.interfaces.IDeviceController.UpsideDownStatusCallback;
import com.htc.gc.interfaces.IGCService;
import com.htc.gc.internal.GCTask;
import com.htc.gc.internal.Protocol;

public class GetUpsideDownStatusTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_UPSIDE_DOWN_STATUS;
	
	private final IDeviceController 		mThat;
	private final UpsideDownStatusCallback 	mCallback;
	
	public GetUpsideDownStatusTask(IDeviceController that, UpsideDownStatusCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, UpsideDownStatus.getKey(bodyBuffer.get()));
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
