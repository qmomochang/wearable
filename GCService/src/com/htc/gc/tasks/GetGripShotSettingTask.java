...
import com.htc.gc.interfaces.IDeviceController.GetGripShotSettingCallback;
...
public class GetGripShotSettingTask extends GCTask {
	private static final int COMMAND_ID = Protocol.SYS_GET_GRIPSHOOT;

	private final IDeviceController mThat;
	private final GetGripShotSettingCallback mCallback;
	
	public GetGripShotSettingTask(IDeviceController that, GetGripShotSettingCallback callback) {
		mThat = that;
		mCallback = callback;
	}
	...
	public void response(InputStream stream, IGCService.ICommandCancel cancel) throws Exception {
		try {
			...
			try {
				mCallback.result(mThat, GripShotSetting.getKey(bodyBuffer.get()));
			}
			catch(Exception e) {
				e.printStackTrace();
				mCallback.error(e);
